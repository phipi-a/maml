...
def positivedef_matrix_sqrt(array):
  ...
  """
  #xx_eigv = np.linalg.eigvals(array)
  # print(np.any(np.isnan(sigma_xx)), np.any(np.isnan(sigma_yy)))
  #print("aa", np.any(xx_eigv <= 0), xx_eigv[xx_eigv <= 0])

  w, v = np.linalg.eig(array)
  #  A - np.dot(v, np.dot(np.diag(w), v.T))
  # print(np.any(np.isnan(sigma_xx)), np.any(np.isnan(sigma_yy)))
  #print("xx", np.any(w <= 0), w[w <= 0])

  wsqrt = np.sqrt(w)
  #print(np.iscomplexobj(array),np.iscomplexobj(w),np.iscomplexobj(v),np.iscomplexobj(wsqrt))
...
def compute_ccas(sigma_xx, sigma_xy, sigma_yx, sigma_yy, epsilon,
  ...
  # inv_xx += epsilon * np.eye(inv_xx.shape[0])
  # inv_yy += epsilon * np.eye(inv_yy.shape[0])


  if verbose:
    print("taking square root")
  invsqrt_xx = np.real(positivedef_matrix_sqrt(inv_xx))
  invsqrt_yy = np.real(positivedef_matrix_sqrt(inv_yy))
  # - gives NAN errors
  ...
  if verbose:
    ...
    #print(arr)
  #print(np.any(np.isnan(arr)),np.any(np.isinf(arr)))
  #print(np.any(np.iscomplex(arr)))
...
def get_cca_similarity(acts1, acts2, epsilon=0., threshold=0.98,
  ...
  z = compute_ccas(sigmaxx, sigmaxy, sigmayx, sigmayy,
                                  epsilon=epsilon,
                                  verbose=verbose)
